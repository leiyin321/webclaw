import { RUN_JS_RPC_METHODS, RUN_JS_RUNTIMES } from "./run-js-policy.js";
...
const BUILTIN_TOOL_DEFINITIONS = [
  ...
  definition("page_file_input", "page", "Set one VFS file on a file input in the active page and dispatch input/change events.",
    ...
    { hostPermissions: ["active_tab_origin"] }),
  definition("run_js", "page", "Execute JavaScript in one explicit, non-cumulative runtime. compute runs isolated calculations without approval. page-isolated runs ordinary window/document code in Chrome's USER_SCRIPT world. page-main runs ordinary page code in the website MAIN world. extension runs a Manifest Sandbox controller with only explicitly declared webclaw.vfs, webclaw.http, and webclaw.chrome RPC methods. Page runtimes never expose extension RPC, and extension never exposes page globals. Provide exactly one of code or vfsPath and return JSON-serializable data.",
    object({
      code: string({ maxLength: 200000, description: "Complete async-function body, at most 200,000 characters. Top-level await and return are supported. In page runtimes use window/document directly. In extension use only declared webclaw RPC methods. Provide code or vfsPath, never both." }),
      vfsPath: string({ description: "Absolute VFS path to a complete .js, .mjs, or .cjs body. It executes in the selected runtime. Provide code or vfsPath, never both." }),
      runtime: string({
        enum: RUN_JS_RUNTIMES,
        description: "Mutually exclusive execution environment: compute for isolated calculation; page-isolated for DOM in USER_SCRIPT; page-main for website JS globals in MAIN; extension for explicitly allowlisted WebClaw RPC."
      }),
      ...
      timeoutMs: integer({ minimum: 100, maximum: 120000, description: "Maximum time WebClaw waits for completion; default 30000. Stopping or timing out page code cannot roll back page-side effects that already started." }),
      target: object({
        tab: string({ enum: ["active"], description: "Use the active tab." }),
        tabId: integer({ minimum: 0, description: "Use one explicit tab ID instead of tab=active." })
      }, [], { description: "Required only for page-isolated and page-main. Provide tab=active or tabId, not both." }),
      capabilities: object({
        methods: array(string({ enum: RUN_JS_RPC_METHODS }), { description: "Exact extension RPC methods used by the script. Required for extension. Wildcards are not supported." }),
        vfs: object({
          read: array(string(), { description: "Absolute readable VFS paths/scopes ending in /* or /**. Required when a declared method reads VFS." }),
        ...
        }, [], { description: "Scopes for declared webclaw.vfs methods. Read and write are checked independently." }),
        network: object({
          origins: array(string(), { description: "Required for http.request, such as https://api.example.com or https://*.example.com/*. Redirects cannot escape this scope." })
        }, [], { description: "Declared HTTP(S) origins for webclaw.http.request." })
      }, [], { description: "Accepted only by extension. Every RPC requires both an exact methods entry and its resource scope." })
    }, ["runtime"]),
    {
      runtime: "page-isolated",
      code: "const ok = window.confirm('Continue?'); return { answer: ok ? 'yes' : 'no', title: document.title };",
      target: { tab: "active" }
    },
    write("sandbox:declared-capabilities", "unknown", "interactive"),
    { permissions: ["userScripts"], hostPermissions: ["active_tab_origin", "args.capabilities.network.origins"], advanced: true }),
