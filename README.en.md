...
- Unified Tool Registry: built-in names, JSON Schemas, permissions, risk, scheduling, and Settings metadata have one source of truth; arguments are recursively validated and model-facing results use a stable envelope.
- Browser tools: structured page snapshots, actions, waits, extraction, screenshots, origin storage, VFS file-input attachment, full tab lifecycle, page translation, background HTTP requests, and approval-gated JavaScript execution.
- Optional browser capabilities: tab groups, recent sessions, downloads, bookmarks, history, clipboard, and local notifications are enabled per Tool and request matching Chrome optional permissions only when needed. Clipboard access is split into `browser_clipboard_read` and `browser_clipboard_write`, so read-only work never requests write permission.
- Virtual filesystem: the file manager and agent tools share an IndexedDB-backed filesystem with structured reads and writes, search, glob, hash, diff, patch, archives, preview, and consolidated `fs_manage` and `fs_trash` operations.
...
- Browser tools: `page_snapshot`, `page_action`, `page_wait`, `page_extract`, `page_screenshot`, `page_storage`, `page_file_input`, and `browser_tabs` provide verified page and tab operations, while `run_js` remains an approval-gated advanced escape hatch.
...
Each user message creates an AgentRun with an initially empty task stack; WebClaw does not create a root Task automatically. The model calls `task_push` only when a separable unit of work benefits from an independent context. Each Task receives only its explicit `instruction`, structured `context`, working directory, and `outputSchema`; the calling Agent or parent Task waits synchronously for the structured result. A Task may call `task_push` again. The defaults allow four levels and sixteen tasks per AgentRun. Settings can raise those values and set an explicit-task-tree model-step budget; zero means unlimited. Ordinary root AgentRun model steps do not consume the Task budget. `task_stack` reports only explicit active Tasks and the shared budget.

Tasks are never added to Tool configuration and do not become Workflows. Their full execution context is removed from the active stack after completion or failure; the caller receives a standard result envelope whose `output` is locally validated against the JSON Schema, then gets at least one model turn to integrate the result before deciding whether the current Agent is complete. When `task_push` occurs on the last regular step, WebClaw reserves this integration turn, but the reserved turn cannot bypass the step limit to execute more Tools. Active snapshots and recent status, count, budget, and error summaries stay in the current Chrome profile for interruption diagnostics without duplicating final answers. WebClaw does not automatically replay uncertain external operations after a service-worker interruption.
...
- `run_js` has four mutually exclusive environments: approval-free isolated `compute`, direct USER_SCRIPT `page-isolated`, direct MAIN-world `page-main`, and allowlisted-RPC `extension`. They are not cumulative permission levels. Page and extension execution is disabled by default and requires approval.
- Every non-compute ad-hoc run shows its runtime, RPC scopes or page target, and source for approval. A Schedule can remember an exact operation; changing its Schedule, runtime, capabilities, page target, or code requires another approval.
...
- Both page runtimes require Chrome 135 or newer and execute ordinary `window`/`document` code directly through `userScripts.execute()`; the nested page RPC has been removed. In Chrome 138+, enable `Allow User Scripts` for WebClaw on the extension details page if Chrome reports that the API is unavailable.
- `extension` exposes only `webclaw.*`. Every RPC validates the exact declared method, VFS path, network origin, and Chrome optional permission. Control APIs such as `identity`, `storage`, `runtime`, `permissions`, `scripting`, and `userScripts` are never exposed. See [run_js execution runtimes](docs/run-js-runtimes.md).
- `run_js` accepts inline `code` or `vfsPath` for a VFS `.js`, `.mjs`, or `.cjs` file; provide exactly one. Approval-free `compute` accepts inline code only.
...
- `fs_manage` provides mkdir, move, copy, touch, and recoverable deletion; `fs_trash` provides list, restore, purge, and empty. Restore rejects a conflicting destination by default, supports `onConflict: "rename"`, and can move the existing destination to trash when `confirmOverwrite: true`; purge and empty operate only on `/.trash` and require `confirm: true`.
